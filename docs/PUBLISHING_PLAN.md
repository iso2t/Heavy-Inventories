...
Status: design and local changelog scaffolding prepared. The user selected automatic publication on version-tag pushes and one changelog file per version. Implementation and platform setup remain pending. No publishing workflow, hosting projects, credentials, repository settings, commits, tags, or pushes are being created by this design pass. Treat all platform setup as absent until verified.
...
Confirmed by the user: version-tag pushes authorize automatic publication, manual runs default to dry-run, and changelogs use one Markdown file per version with an Unreleased draft. The remaining implementation defaults below are the proposed design for discussion.

| Decision | Proposed behavior |
| --- | --- |
| Publication trigger | A maintainer pushes an existing release tag such as `v26.1.0.0-rc.1`; ordinary branch pushes and pull requests only validate |
...
Display names include version and loader, for example `Heavy Inventories 26.1.0.0-rc.1 (Fabric)`. Give Modrinth entries distinct version numbers, `<version>+fabric` and `<version>+neoforge`; the embedded mod version stays `<version>`. Release classification is derived before adding that platform suffix. Modrinth exposes file, loader, channel, and dependency fields per version. [Modrinth version API](https://docs.modrinth.com/api/operations/createversion/)

| Base mod version | CurseForge / Modrinth channel | GitHub prerelease |
| --- | --- | --- |
| `26.1.0.0` | release | false |
| `26.1.0.0-rc.1` | beta | true |
| `26.1.0.0-beta.1` | beta | true |
| `26.1.0.0-alpha.1` | alpha | true |

Reject other suffixes until explicitly supported. The existing four-component version is intentional; do not rely on a strict three-component SemVer parser or silently rename it. Prereleases must not become GitHub's latest stable release. Proposed latest policy: configure one current Minecraft release line (initially `26.1`) in `release/publishing.json`; only newer stable versions on that line can become latest. Compare the numeric version components, not strings. Older-line maintenance releases remain downloadable without displacing the selected latest line.

Declare Fabric API and Cloth Config as required for Fabric. NeoForge requires Cloth Config on clients but not dedicated servers: use the closest truthful platform dependency representation and explicitly describe the client-only requirement in release notes. Neither upload API's simple dependency relation represents that distinction fully. Do not falsely label a client requirement as universally optional. Keep platform project IDs/slugs for dependencies explicit and validate them during setup.

## Setup from zero

Follow the [publishing setup guide](PUBLISHING.md) for token creation, exact repository settings, changelog preparation, and tag commands. The guide is available now; executable publishing remains pending.
...
A publish job consumes only this validated bundle and rechecks its hashes. Required IDs, tokens, channel support, game/loader tags, project access, and dependency mappings are checked before the first upload. Missing configuration fails the whole live attempt before creating a draft or uploading files. Platform moderation may still occur after an accepted upload; report that status distinctly from public availability.

Prefer a small repository-owned orchestration script with narrow CurseForge, Modrinth, and GitHub adapters. Keep network operations out of normal Gradle `build` tasks. Direct API adapters provide explicit duplicate detection and receipts; a convenience upload action should only replace them if it satisfies those same recovery rules. Pin workflow actions and script dependencies during implementation.
...
3. On rerun, use the original bundle from the draft, verified against its manifest. Do not rebuild and assume byte identity. Check confirmed remote entries and skip matching uploads; fail on conflicts. Missing destinations alone are candidates for upload.
...
5. A maintainer can supply an existing remote file/version ID for reconciliation; validate it belongs to the expected project and corresponds to the candidate. Do not provide a generic force-overwrite switch. If certainty cannot be established, leave the run incomplete.
...
Use ordinary Markdown and the same rendered body for all platforms. Convert supported repository-relative documentation links to absolute links pinned to the release commit, or require absolute links; reject broken links in the preview. Avoid local filesystem paths, task links, raw HTML, and platform-specific embeds. Missing or ambiguous release notes must block publication rather than falling back to the README or a stale changelog.

The local `26.1.0.0-rc.1.md` draft starts from the current release notes. Its presence does not publish or schedule RC1.
...
- [ ] Agree the client-only Cloth dependency representation on NeoForge, prerelease mapping, and latest-stable policy.
...
- [ ] Add scripts validating properties, tag, changelog, compatibility, dependency mappings, and manifest schema.
- [ ] Generate a preview and exact artifact list; add validation to ordinary CI without secrets.
- [ ] Test missing/empty notes, wrong version/tag, prerelease classification, unsupported metadata, and accidental extra jar selection.

Acceptance: invalid releases fail locally/CI before any upload is possible.

### 3. Build and package in dry-run mode

- [ ] Reuse the Linux/Windows build matrix and gate packaging on both results for the selected SHA.
- [ ] Upload the canonical Linux bundle, rendered changelog, checksums, and planned destination summary as workflow artifacts.
- [ ] Implement dry-run as a path with no publishing credentials and no external writes.
...
- [ ] Implement authenticated preflight, GitHub draft staging, four loader-specific uploads, and GitHub finalization.
- [ ] Implement durable intents/receipts, remote reconciliation, hash-conflict rejection, and matching-release no-ops.
- [ ] Test every partial-failure position with mocked APIs, including upload succeeded but response/receipt was lost.
- [ ] Add concurrency, bounded read retries, rate-limit handling, and sanitized failure summaries.
...
- [ ] Verify the setup guide's release and recovery procedures against the implemented workflow, including final input names.
...
Design, setup instructions, and changelog scaffolding exist locally now. Executable files below are future work:
...
Official documentation checked on 2026-09-25 is linked beside the relevant platform details. Recheck API schemas and choose pinned tool versions when implementing the adapters.
