...
class ResolvedWeightsTest {
	...
	private static WeightPackData.Result pack (Map<String, WeightDefinition> entries) {
		...
		entries.forEach((name, value) -> definitions.put(id(name), new WeightPackData.Entry(value, id("heavyinventories/weights/" + name + ".json"), "test-pack")));
		return new WeightPackData.Result(definitions, List.of(), List.of());
	}

	private static Set<Identifier> items (String... names) {
		return new HashSet<>(Arrays.stream(names).map(ResolvedWeightsTest::id).toList());
	}

	@Test
	void materialWeightsPropagateWithoutTurningDerivedValuesIntoAnchors () {
	...
	@Test
	void explicitZeroAndExplicitProductOverrideRecipes () {
		var data = pack(Map.of("log", new WeightDefinition.Fixed(8), "planks", new WeightDefinition.Fixed(0), "stick", new WeightDefinition.Fixed(0.053125f)));
		var result = ResolvedWeights.resolve(data, List.of(recipe("planks", 4, "log"), recipe("stick", 4, "planks", "planks")), items("log", "planks", "stick"));
	...
	@Test
	void inferMarkerAndRemovedDefinitionRestoreDerivedWeights () {
	...
	@Test
	void allRegisteredItemsGetWeightsAndFallbackCanFeedRecipes () {
		var result = ResolvedWeights.resolve(pack(Map.of()), List.of(recipe("arrow", 4, "flint", "feather", "stick")), items("arrow", "flint", "feather", "stick", "unknown"));
	...
	@Test
	void invalidDatapackCannotBecomeFallbackSuccess () {
		var data = new WeightPackData.Result(Map.of(), List.of(new WeightPackData.Problem("test:heavyinventories/weights/log.json", "broken-pack", "Invalid weight")), List.of());
		var error = assertThrows(IllegalArgumentException.class, () -> ResolvedWeights.resolve(data, List.of(), items("log")));
		assertTrue(error.getMessage().contains("broken-pack"));
		assertTrue(error.getMessage().contains("weights/log.json"));
	}

	@Test
	void provenanceDistinguishesEqualValuesAndRecordsWinningDefinition () {
		var data = pack(Map.of("fixed", new WeightDefinition.Fixed(0.1f), "derived", WeightDefinition.Infer.INSTANCE));
		var result = ResolvedWeights.resolve(data, List.of(recipe("derived", 1, "raw"), recipe("cycle", 1, "cycle")), items("fixed", "derived", "raw", "cycle"));
	...
	@Test
	void compressionCyclesRequireAnchorsAndRespectOutputCounts () {
		var recipes = List.of(recipe("ingot", 9, "block"), recipe("block", 1, "ingot", "ingot", "ingot", "ingot", "ingot", "ingot", "ingot", "ingot", "ingot"));
