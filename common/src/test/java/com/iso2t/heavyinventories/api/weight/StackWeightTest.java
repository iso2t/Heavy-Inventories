...
class StackWeightTest {
    ...
    @Test void shellContentsAndOuterCountAreCountedOnce() {
    ...
    @Test void componentMutationChangesWeightAndBothContentComponentsCount() {
    ...
    @Test void depthAndWorkLimitsNeverPublishAnUnderestimatedTotal() {
        ...
        wide.set(DataComponents.BUNDLE_CONTENTS, new BundleContents(
                Collections.nCopies(StackWeight.MAX_VISITS, new ItemStackTemplate(Items.STONE))));
    ...
    @Test void missingInvalidOrOverflowingDefinitionsDoNotProduceNaNOrPartialWeight() {
        ...
        assertEquals(0, StackWeight.of(ItemStack.EMPTY, id -> { throw new AssertionError(); }).weight());
        assertEquals(0, StackWeight.of(new ItemStack(Items.STONE), id -> 0).weight());
    }

    @Test void calculationsRetainFractionsUnderCommaDecimalLocale() {
