...
public final class ServerConfigScreen {
	...
	public static ConfigBuilder create (java.util.function.Consumer<ServerConfigUpdatePayload> send) {
		...
		var capacity = entries.startFloatField(Component.translatable("option.heavyinventories.starting_max_weight"), initial.startingWeight()).setDefaultValue(ServerSettings.DEFAULT.startingWeight()).setMin(Float.MIN_VALUE).setMax(ServerSettings.MAX_VALUE).setTooltip(Component.translatable("option.heavyinventories.starting_max_weight.tooltip")).setErrorSupplier(value -> {
			try {
				new ServerSettings(value);
		...
		var mode = entries.startEnumSelector(Component.translatable("option.heavyinventories.walking_mode"), WalkingMode.class, initial.walkingMode()).setDefaultValue(ServerSettings.DEFAULT.walkingMode()).setEnumNameProvider(value -> Component.translatable("option.heavyinventories.walking_mode." + ((WalkingMode) value).id())).setTooltip(Component.translatable("option.heavyinventories.walking_mode.tooltip")).build();
	...
	private static void addEffectField (ConfigBuilder builder, ConfigCategory category, JsonObject values, JsonObject defaults, String group, String key, boolean editable, List<Runnable> readFields) {
		String translation = "option.heavyinventories.effects." + group + "." + key;
		var entries = builder.entryBuilder();
		if (values.get(key).getAsJsonPrimitive().isBoolean()) {
			var field = entries.startBooleanToggle(Component.translatable(translation), values.get(key).getAsBoolean()).setDefaultValue(defaults.get(key).getAsBoolean()).setTooltip(Component.translatable(translation + ".tooltip")).build();
			field.setEditable(editable);
			readFields.add(() -> values.addProperty(key, field.getValue()));
			category.addEntry(field);
		} else {
			var field = entries.startFloatField(Component.translatable(translation), values.get(key).getAsFloat()).setDefaultValue(defaults.get(key).getAsFloat()).setMin(0).setMax(ServerSettings.MAX_VALUE).setTooltip(Component.translatable(translation + ".tooltip")).setErrorSupplier(value -> {
				readFields.forEach(Runnable::run);
				values.addProperty(key, value);
				var candidate = new JsonObject();
				candidate.add(group, values.deepCopy());
				try {
					EffectsSettings.parse(candidate);
					return Optional.empty();
				} catch (IllegalArgumentException e) {
					return Optional.of(Component.literal(e.getMessage()));
				}
