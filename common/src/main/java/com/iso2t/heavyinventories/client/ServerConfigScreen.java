...
public final class ServerConfigScreen {
	...
	public static ConfigBuilder create (java.util.function.Consumer<ServerConfigUpdatePayload> send) {
		...
		var field = entries.startFloatField(Component.translatable("option.heavyinventories.starting_max_weight"), initial).setDefaultValue(ServerSettings.DEFAULT.startingWeight()).setMin(Float.MIN_VALUE).setMax(ServerSettings.MAX_VALUE).setErrorSupplier(value -> {
			try {
				new ServerSettings(value);
				return java.util.Optional.empty();
			} catch (IllegalArgumentException e) {
				return java.util.Optional.of(Component.literal(e.getMessage()));
			}
		}).setSaveConsumer(value -> edited[0] = value).build();
		field.setEditable(holder.canEditServerConfig());
		category.addEntry(field);
		var modeField = entries.startEnumSelector(Component.translatable("option.heavyinventories.walking_mode"), WalkingMode.class, initialMode).setDefaultValue(ServerSettings.DEFAULT.walkingMode()).setEnumNameProvider(mode -> Component.translatable("option.heavyinventories.walking_mode." + ((WalkingMode) mode).id())).setTooltip(Component.translatable("option.heavyinventories.walking_mode.tooltip")).setSaveConsumer(mode -> editedMode[0] = mode).build();
		modeField.setEditable(holder.canEditServerConfig());
		category.addEntry(modeField);
		category.addEntry(entries.startTextDescription(Component.translatable(holder.canEditServerConfig() ? "config.heavyinventories.edit_help" : "config.heavyinventories.read_only")).build());
