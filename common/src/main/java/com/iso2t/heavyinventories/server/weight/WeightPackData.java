...
public final class WeightPackData {
    ...
    public record Problem(String resource, String sourcePack, String message) {
        @Override public String toString() { return resource + " [pack " + sourcePack + "]: " + message; }
    }
    ...
    public static Result load(ResourceManager manager, Predicate<Identifier> registeredItem) {
        ...
        if (resources.size() > MAX_DEFINITIONS)
            return new Result(Map.of(), List.of(new Problem(DIRECTORY, "multiple", "Too many weight definitions (limit " + MAX_DEFINITIONS + ")")), List.of());
    ...
    public static Identifier itemId(Identifier resource) {
        ...
        if (!path.startsWith(prefix) || !path.endsWith(".json"))
            throw new IllegalArgumentException("Not an item-weight resource: " + resource);
