...
public final class WeightReport {
    ...
    public static JsonObject create(String namespace, long revision, Map<Identifier, Float> weights,
                                    Map<Identifier, WeightProvenance> sources) {
        ...
        weights.keySet().stream().filter(id -> id.getNamespace().equals(namespace))
                .sorted(Comparator.comparing(Identifier::toString)).forEach(id -> {
