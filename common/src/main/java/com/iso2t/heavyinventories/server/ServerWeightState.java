...
public final class ServerWeightState {
	...
	public static void start (MinecraftServer server) {
		...
		warnLegacyFiles();
		HeavyInventories.LOGGER.info("Initialized {} item weights ({} explicit datapack anchors)", state.weights.size(), state.explicitWeights.size());
	}
	...
	private static void warnLegacyFiles () {
		...
		try (var files = Files.list(directory)) {
			...
			if (count > 0) HeavyInventories.LOGGER.warn("Ignoring {} legacy weight JSON files in {}. Use /heavyinventories convert legacy <pack_name> to export a reviewable datapack; original files were preserved.", count, directory);
		} catch (IOException e) {
	...
	public void replace (ServerSettings settings, Map<Identifier, Float> values) {
		var sources = new HashMap<Identifier, WeightProvenance>();
		values.forEach((id, value) -> sources.put(id, value.equals(weights.get(id)) ? provenance.getOrDefault(id, WeightProvenance.SESSION) : WeightProvenance.SESSION));
		var anchors = new HashMap<Identifier, Float>();
		explicitWeights.forEach((id, value) -> {
			if (value.equals(values.get(id))) anchors.put(id, value);
		});
	...
	private void replace (ServerSettings settings, Map<Identifier, Float> values, Map<Identifier, Float> explicitWeights, Map<Identifier, WeightProvenance> sources) {
		...
		for (int i = 0; i < chunks; i++) {
			...
			next.add(new ItemWeightsPayload(revision + 1, i, chunks, entries.subList(from, Math.min(from + ItemWeightsPayload.CHUNK_SIZE, entries.size()))));
		}
	...
	public float weight (ItemStack stack) {
		return StackWeight.of(stack, this::unitWeight).weight();
	}

	public float unitWeight (Identifier item) {
