...
public sealed interface WeightDefinition {
    ...
    record Fixed(float weight) implements WeightDefinition {
        public Fixed { ServerSettings.validateItemWeight(weight); }
    }

    enum Infer implements WeightDefinition { INSTANCE }
    ...
    static WeightDefinition parse(Reader source) throws IOException {
        ...
        while ((count = source.read(buffer)) != -1) {
            if (text.length() + count > MAX_DOCUMENT_CHARS)
                throw new IllegalArgumentException("Weight definition exceeds " + MAX_DOCUMENT_CHARS + " characters");
        ...
        try (var json = new JsonReader(new StringReader(text.toString()))) {
            ...
            switch (field) {
                case "weight" -> {
                    ...
                    if (decimal.signum() < 0 || decimal.compareTo(BigDecimal.valueOf((long) ServerSettings.MAX_VALUE)) > 0)
                        throw new IllegalArgumentException("weight must be between 0 and 1000000000 pounds");
                    float value = decimal.floatValue();
                    if (decimal.signum() > 0 && value == 0)
                        throw new IllegalArgumentException("Positive weight is too small to represent; use explicit zero if intended");
            ...
            if (json.hasNext()) {
                ...
                throw new IllegalArgumentException(extra.equals(field)
                        ? "Duplicate weight-definition field: " + extra
                        : "Expected exactly one field; unexpected field: " + extra);
            }
            json.endObject();
