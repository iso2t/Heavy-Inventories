...
public abstract class EntityMovementMixin {
    @ModifyExpressionValue(method = "moveRelative", at = @At(value = "INVOKE",
    ...
    private Vec3 heavyinventories$scaleMovement(Vec3 movement) {
        ...
        float multiplier = player.isInWater() || player.isInLava()
                ? holder.getFluidSwimMultiplier() : holder.getWalkingMultiplier();
