...
public final class EncumbranceEffects {
	...
	public record State(float exhaustionMultiplier, float walkingCostPerBlock, float fallMultiplier,
	                    float swimmingMultiplier, float sinkingMultiplier, boolean deniesUpwardMovement, float knockbackResistance) {
	...
	public static State calculate (float weight, float capacity, WalkingMode mode, EffectsSettings settings,
	                               Fluid fluid, boolean allExempt, boolean locomotionExempt) {
		if (!Float.isFinite(weight) || weight < 0 || !Float.isFinite(capacity) || capacity <= 0 || mode == null || settings == null || fluid == null)
			throw new IllegalArgumentException("Invalid encumbrance effect input");
		...
		return new State(
				exertion ? (float) (1 + (e.maxMultiplier() - 1) * burden) : 1,
				exertion ? (float) (e.walkingCostPerBlock() * burden) : 0,
				f.enabled() ? (float) (1 + (f.maxMultiplier() - 1) * ramp(ratio, f.startPercent(), f.fullPercent())) : 1,
				inFluid && w.enabled() ? (float) (1 - (1 - w.minMultiplier()) * ramp(ratio, w.startPercent(), w.fullPercent())) : 1,
				inFluid && s.enabled() ? (float) (1 + (s.maxMultiplier() - 1) * ramp(ratio, s.startPercent(), s.fullPercent())) : 1,
				inFluid && u.enabled() && ratio * 100 >= u.thresholdPercent(),
				k.enabled() && !unknown ? (float) (k.maxResistance() * Math.clamp((double) weight / k.referenceWeight(), 0, 1)) : 0);
	}
