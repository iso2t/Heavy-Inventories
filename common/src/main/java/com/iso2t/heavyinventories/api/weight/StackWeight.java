...
public final class StackWeight {
    ...
    public static Result total(List<ItemStack> stacks, ToDoubleFunction<Identifier> definitions) {
        ...
        for (var stack : stacks) {
            ...
            if (!calculation.complete || !Double.isFinite(total) || total >= TOO_COMPLEX)
                return new Result(TOO_COMPLEX, false);
        }
    ...
    private static final class Calculation {
        ...
        private Calculation(ToDoubleFunction<Identifier> definitions) { this.definitions = definitions; }

        private double visit(ItemInstance stack, int depth) {
            if (!complete) return 0;
            if (--remaining < 0 || depth > MAX_DEPTH) { complete = false; return 0; }
            ...
            if (!Double.isFinite(weight) || weight < 0) { complete = false; return 0; }
