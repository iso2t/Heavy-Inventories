...
public final class PlayerHolder {
    ...
    private float bracingOffset, reinforcedOffset, strengthOffset;
    ...
    public PlayerHolder(Player player) { this.player = player; }
    ...
    public void update() {
        ...
        var calculated = Encumbrance.calculate(weight, baseCapacity, level(ModEnchantments.BRACING),
                level(ModEnchantments.REINFORCED), strength == null ? 0 : (int) Math.min(256L, 1L + strength.getAmplifier()),
                level(ModEnchantments.SUREFOOTED), walkingMode, player.isCreative() || player.isSpectator());
    ...
    private int level(ResourceKey<Enchantment> key) {
        return player.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).get(key)
                .map(enchantment -> EnchantmentHelper.getEnchantmentLevel(enchantment, player)).orElse(0);
    }

    PlayerWeightCache weightCache() { return weightCache; }
    public Player getPlayer() { return player; }
    public float getWeight() { return weight; }
    public float getBaseMaxWeight() { return baseCapacity; }
    public float getBracingOffset() { return bracingOffset; }
    public float getReinforcedOffset() { return reinforcedOffset; }
    public float getStrengthOffset() { return strengthOffset; }
    public float getMaxWeight() { return baseCapacity + bracingOffset + reinforcedOffset + strengthOffset; }
    public WalkingMode walkingMode() { return walkingMode; }
    ...
    private boolean exempt() { return player.isCreative() || player.isSpectator() || !hasServerState(); }
    public boolean isEncumbered() { return !exempt() && encumbered; }
    public boolean isOverEncumbered() { return !exempt() && overloaded; }
    ...
    public float getWalkingMultiplier() { return movementExempt() ? 1 : walkingMultiplier; }
    ...
    public void synchronize(ServerPlayer target) {
        ...
        var snapshot = new PlayerWeightPayload(target.getId(), target.level().dimension().identifier(),
                weight, baseCapacity, bracingOffset, reinforcedOffset, strengthOffset, walkingMultiplier, walkingMode,
                isEncumbered(), isOverEncumbered(), ServerConfiguration.canEdit(target), state.revision());
