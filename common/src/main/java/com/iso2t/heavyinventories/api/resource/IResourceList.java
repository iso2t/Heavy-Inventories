...
public final class IResourceList {
	...
	public static List<RecipeWeights.Recipe> snapshot (Level level) {
		...
		for (var holder : server.getRecipeManager().getRecipes()) {
			...
			if (type != ShapedRecipe.class && type != ShapelessRecipe.class && type != SmeltingRecipe.class && type != BlastingRecipe.class && type != SmokingRecipe.class && type != CampfireCookingRecipe.class && type != StonecutterRecipe.class)
				continue;
			...
			for (var display : recipe.display()) {
				if (!(display.result() instanceof SlotDisplay.ItemStackSlotDisplay) && !(display.result() instanceof SlotDisplay.ItemSlotDisplay)) continue;
