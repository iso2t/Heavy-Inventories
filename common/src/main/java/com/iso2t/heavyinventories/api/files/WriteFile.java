...
public final class WriteFile {
	...
	public static JsonObject readWeights (Path path) throws IOException {
		...
		for (var entry : root.entrySet()) {
			if (!entry.getValue().isJsonObject()) throw new IllegalArgumentException("Invalid weight entry " + entry.getKey() + " in " + path);
			var weight = entry.getValue().getAsJsonObject().get("weight");
			if (weight != null) {
				if (!weight.isJsonPrimitive() || !weight.getAsJsonPrimitive().isNumber()) throw new IllegalArgumentException("Invalid weight value " + entry.getKey() + " in " + path);
