...
public final class Encumbrance {
    private Encumbrance() {}
    public record State(float bracing, float reinforced, float strength, float capacity,
                        boolean encumbered, boolean overloaded, float walkingMultiplier) {}

    public static State calculate(float weight, float base, int bracingLevel, int reinforcedLevel,
                                  int strengthLevel, int surefootedLevel, WalkingMode mode, boolean exempt) {
        ...
        double multiplier = exempt ? 1 : mode == WalkingMode.PROGRESSIVE
                ? Math.sqrt(Math.max(0, 1 - ratio))
                : Math.clamp((1 - ratio) / 0.1, 0, 1);
